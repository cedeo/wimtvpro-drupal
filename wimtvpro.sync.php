...
if ($array_json_videos == NULL) {
    ...
    if ($num > 0) {
        ...
        foreach ($array_all_videos as $video) {
            if (isset($video["acquiredIdentifier"]))
            ...
            $url_thumbs = check_plain('<img src="' . $video["thumbnailUrl"] . '"  title="' . $title . '" class="wimtv-thumbnail" />');
            ...
            if (trim($content_item) != "") {
                ...
                if (!$trovato) {
                    $query = db_query("INSERT INTO {wimtvpro_videos} (uid, contentidentifier, mytimestamp, position, state, viewVideoModule, status, acquiredIdentifier, urlThumbs, urlPlay, category, title, duration, showtimeIdentifier) VALUES ('" . variable_get("userWimtv") . "','" . $content_item . "','" . time() . "',0,'" . $pos_wimtv . "','3','" . $status . "','" . $acquired_identifier . "','" . $url_thumbs . "','" . $url_video2 . "','" . $categories . "','" . $title . "','" . $duration . "','" . $showtime_identifier . "')");
                } else {
